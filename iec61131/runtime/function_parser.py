...
class FunctionProcessor:
    ...
    def __init__(self):
        ...
        self.function_header_pattern = r'FUNCTION\s+(\w+)(?:\s*:\s*(\w+))?\s*(?:\([^)]*\))?\s*(?:\(\*([^)]*)\*\))?'
        self.var_section_pattern = r'(VAR(?:_INPUT|_OUTPUT|)\s*.*?\s*END_VAR)'
        self.var_content_pattern = r'VAR(?:_INPUT|_OUTPUT|)\s*(.*?)\s*END_VAR'
        
    def extract_function_declarations(self, template_content: str) -> Dict[str, FunctionDeclaration]:
        """
    ...
    def strip_var_sections(self, template_content: str) -> str:
        ...
        """
        # Remove all VAR sections (VAR, VAR_INPUT, VAR_OUTPUT) using a simpler approach
        ...
        for line in lines:
            ...
            if stripped.startswith('VAR') and (stripped == 'VAR' or stripped.startswith('VAR_INPUT') or stripped.startswith('VAR_OUTPUT')):
                in_var_block = True
                continue
        ...
        result = re.sub(function_pattern, r'\1\2', result, flags=re.MULTILINE | re.IGNORECASE)
    ...
    def generate_fun_from_template(self, template_content: str, tokens: Dict[str, str]) -> str:
        """
    ...
    def _extract_function_declaration_only(self, block: str, tokens: Dict[str, str]) -> str:
    ...
    def _find_function_blocks(self, content: str) -> List[str]:
        """Find all FUNCTION...END_FUNCTION blocks"""
    ...
    def _parse_var_section(self, block: str, var_type: str) -> List[Parameter]:
        ...
        pattern = fr'{var_type}\s*(.*?)\s*END_VAR'
        ...
        for line in var_content.split('\n'):
            ...
            var_match = re.match(r'(\w+)\s*:\s*([^;]+)(?:\s*;\s*(?:\(\*([^)]*)\*\))?)?', line)
            if var_match:
    ...
    def _extract_metadata(self, description: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    def _generate_single_fun_declaration(self, func: FunctionDeclaration, tokens: Dict[str, str]) -> str:
